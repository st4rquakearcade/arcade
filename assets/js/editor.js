(function () {
  ...
  function bind() {
    ...
    function doSave(redirect) {
      var post = collectPost();
      ...
      SQPosts.save(post).then(function (saved) {
        editId = saved.id;
        status(redirect ? "저장되었습니다." : "임시 저장되었습니다.");
        if (redirect)
          setTimeout(function () {
    ...
    $("ed-save").addEventListener("click", function () { doSave(true); });
    if ($("ed-draft")) $("ed-draft").addEventListener("click", function () { doSave(false); });
